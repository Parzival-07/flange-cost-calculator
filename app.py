...
if check_password():
    ...
    if flange_database:
        ...
        def selection_on_change_callback(level_idx, widget_key_for_level):
            # This callback ONLY updates the selection for the current dropdown.
            # It does NOT reset subsequent dropdowns' selected values in session_state.
            # The options for subsequent dropdowns will naturally update in the next UI render.
        ...
        for i in range(len(level_names)):
            # Determine the data source for the current dropdown's options
            # by traversing from the root using selections made so far for *previous* levels
            data_source_for_current_dropdown = flange_database
            for prev_level_idx in range(i): # Path to current level's parent
                ...
                if isinstance(data_source_for_current_dropdown, dict) and key_in_path in data_source_for_current_dropdown:
                    ...
                    data_source_for_current_dropdown = {} # Path broken, no valid options
                    break
            
            # Check if this data_source_for_current_dropdown is actually the final data payload
            if isinstance(data_source_for_current_dropdown, dict) and \
               all(k in data_source_for_current_dropdown for k in ['FW', 'CW']):
                # Yes, it is. This means the selections up to level i-1 led to the data.
                # So, we should not render a dropdown for level i.
                break # Stop creating more dropdowns
            ...
            try:
                ...
                # Current selection is not in the (new) list of options.
                # This happens if a previous dropdown changed, making the old selection invalid.
                # Show blank, but keep st.session_state.selections[i] as is until user changes it.
                current_idx = 0 
        ...
        active_selection_path = [s for s in st.session_state.selections if s] # Get only non-empty selections
        ...
        if selected_flange_data and active_selection_path: # active_selection_path ensures we have a key
            ...
            else: # Should ideally not happen if keys are strings
                primary_flange_description_for_display = str(final_key_part)
    
            # Clean up extra_flange_prices for items no longer identified
            current_extra_set = set(identified_extra_flanges)
            # Rebuild the dictionary, only keeping prices for currently identified extra flanges
            # and ensuring their keys exist in session_state for the widgets
            new_extra_flange_prices = {}
            for desc in identified_extra_flanges:
                price_key = f"price_extra_{create_safe_key(desc)}"
                if price_key not in st.session_state: # If a new extra flange appears
        ...
        if selected_flange_data:
            ...
            if display_path_parts and identified_extra_flanges: # If there are extra flanges, use the primary desc
            ...
            if cw_flange is None or fw_flange is None:
                ...
                # All these use their respective keys in st.session_state and will persist
                ...
                if ft_flange_from_data is None or not isinstance(ft_flange_from_data, (int, float)):
                    ...
                    if st.session_state.ft_manual_checkbox:
                        ...
                        ft_flange_final = st.session_state.ft_manual_input # Use manually entered value
                    # If checkbox not ticked, ft_flange_final remains None (or the invalid data value)
                
                if st.button("Calculate Costs", key="calc_button"):
                    # Retrieve all cost inputs from session_state for calculation
                    ...
                    if ft_flange_final is not None and isinstance(ft_flange_final, (int, float)): # Check type for safety
                    ...
                    if st.session_state.extra_flange_prices and identified_extra_flanges:
                        ...
                        for desc in identified_extra_flanges: # Iterate over current identified
                            price = st.session_state.extra_flange_prices.get(desc, 0.0) # Get from our synced dict
